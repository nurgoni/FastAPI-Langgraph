...
@router.get("/users/id/{id}", response_model=ShowUser, status_code=status.HTTP_200_OK)
def fetch_user_with_id(id: UUID, db: Session = Depends(get_db)):
...
@router.get("/users/email/{email}", response_model=ShowUser, status_code=status.HTTP_200_OK)
